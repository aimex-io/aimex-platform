...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Aimex Platform", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Aimex Platform",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Aimex Platform");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Aimex Platform");
    expect(buildDocumentTitle([], null)).toBe("Aimex Platform");
  });
});
